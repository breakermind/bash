...
if $TLS_WWW; then
	if [ -z "$(dig +short "www.$APP_HOST")" ]; then
		echo "[error] Create www.$APP_HOST host A record in domain DNS zone first."
		exit
	fi
fi

if [ -z "$(dig +short "$APP_HOST")" ]; then
	echo "[error] Create $APP_HOST host A record in domain DNS zone first."
	exit
fi
